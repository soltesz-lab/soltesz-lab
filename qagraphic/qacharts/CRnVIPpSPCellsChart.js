var CRnVIPpSPCellsFCN = function() {	
	try {
	var myKey = "CRnVIPpSPCOUPFact";
	...
	nNOSNPYoverlapCOUPTFII = .83; //parseFloat(((document.getElementById(myKey).dataset.custom)/100).toFixed(2)); 
	var COUPTFIIoverlapnNOSNPY = .15;
	window.CRnVIPpSPamidale = Math.round(window.COUPcells*.35*COUPTFIIoverlapnNOSNPY/nNOSNPYoverlapCOUPTFII);
	window.is1_SP = Math.round(window.CRnVIPpSPamidale/10)*10;

	
	if (document.getElementById("CRnVIPpSPCellsChart") && document.getElementById("CRnVIPpSPCells").style.display!="hidden" && document.getElementById("CRnVIPpSPCells").style.display!="none") {
	jQuery('#CRnVIPpSPCellsChart').highcharts({
    chart: {
        renderTo:'container',
        type:'scatter'
    },
    ...
    series: [	{
       ...
       name:'CR-/VIP+ (IS II) SP cells',
       label:Math.round(window.CRnVIPpSPamidale/10)*10,
       data: [{
           x:55,
           y:50,
		   z:Math.round(window.CRnVIPpSPamidale/10)*10,
           marker:{
               radius:Math.sqrt(Math.round(window.CRnVIPpSPamidale/10)*10/(Math.round(window.COUPcells*.35/10)*10+Math.round(window.CRnVIPpSPamidale/10)*10)*2000),
    ...
    },
	{
        ...
        data: [{
           x:45,
            ...
            marker:{
               radius:Math.sqrt(Math.round(window.COUPcells*.35/10)*10/(Math.round(window.COUPcells*.35/10)*10+Math.round(window.CRnVIPpSPamidale/10)*10)*2000),
