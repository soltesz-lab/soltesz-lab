var CRnVIPpSRCellsFCN = function() {	
	try {
	var myKey = "CRnVIPpSRCOUPFact";
	...
	nNOSNPYoverlapCOUPTFII = .81; //parseFloat(((document.getElementById(myKey).dataset.custom)/100).toFixed(2)); 
	var COUPTFIIoverlapnNOSNPY = .06;
	window.CRnVIPpSRamidale = Math.round(window.COUPcells*.22*COUPTFIIoverlapnNOSNPY/nNOSNPYoverlapCOUPTFII);
	window.is1_SR = Math.round(window.CRnVIPpSRamidale/10)*10;

	
	if (document.getElementById("CRnVIPpSRCellsChart") && document.getElementById("CRnVIPpSRCells").style.display!="hidden" && document.getElementById("CRnVIPpSRCells").style.display!="none") {
	jQuery('#CRnVIPpSRCellsChart').highcharts({
    chart: {
        renderTo:'container',
        type:'scatter'
    },
    ...
    series: [	{
       ...
       name:'CR-/VIP+ (IS II) SR cells',
       label:Math.round(window.CRnVIPpSRamidale/10)*10,
       data: [{
           x:55,
           y:50,
		   z:Math.round(window.CRnVIPpSRamidale/10)*10,
           marker:{
               radius:Math.sqrt(Math.round(window.CRnVIPpSRamidale/10)*10/(Math.round(window.COUPcells*.35/10)*10+Math.round(window.CRnVIPpSRamidale/10)*10)*2000),
    ...
    },
	{
        ...
        data: [{
           x:45,
            ...
            marker:{
               radius:Math.sqrt(Math.round(window.COUPcells*.35/10)*10/(Math.round(window.COUPcells*.35/10)*10+Math.round(window.CRnVIPpSRamidale/10)*10)*2000),
