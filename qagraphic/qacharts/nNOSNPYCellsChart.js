var nNOSNPYCellsFCN = function() {	
	...
	window.nNOSNPYoverlapCOUPTFII = .63; //parseFloat(((document.getElementById(myKey).dataset.custom)/100).toFixed(2)); 
	var COUPTFIIoverlapnNOSNPY = .59;
	window.nNOSNPYpyramidale = Math.round(window.COUPcells*.35*COUPTFIIoverlapnNOSNPY/window.nNOSNPYoverlapCOUPTFII);

	
	jQuery('#nNOSNPYCellsChart').highcharts({
    chart: {
        renderTo:'container',
        type:'scatter'
    },
