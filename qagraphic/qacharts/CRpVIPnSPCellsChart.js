var CRpVIPnSPCellsFCN = function() {
	try {
	var myKey = "CRpVIPnSPCOUPFact";
	...
	nNOSNPYoverlapCOUPTFII = .85; //parseFloat(((document.getElementById(myKey).dataset.custom)/100).toFixed(2)); 
	var COUPTFIIoverlapnNOSNPY = .27;
	window.CRpVIPnSPamidale = Math.round(window.COUPcells*.35*COUPTFIIoverlapnNOSNPY/nNOSNPYoverlapCOUPTFII);
	window.is2_SP = Math.round(window.CRpVIPnSPamidale/10)*10;

	
	if (document.getElementById("CRpVIPnSPCellsChart") && document.getElementById("CRpVIPnSPCells").style.display!="hidden" && document.getElementById("CRpVIPnSPCells").style.display!="none") {
	jQuery('#CRpVIPnSPCellsChart').highcharts({
    chart: {
        renderTo:'container',
        type:'scatter'
    },
    ...
    series: [	{
       ...
       name:'CR+/VIP- (IS I) SP cells',
       label:Math.round(window.CRpVIPnSPamidale/10)*10,
       data: [{
           x:55,
           y:50,
		   z:Math.round(window.CRpVIPnSPamidale/10)*10,
           marker:{
               radius:Math.sqrt(Math.round(window.CRpVIPnSPamidale/10)*10/(Math.round(window.COUPcells*.35/10)*10+Math.round(window.CRpVIPnSPamidale/10)*10)*2000),
    ...
    },
	{
        ...
        data: [{
           x:45,
            ...
            marker:{
               radius:Math.sqrt(Math.round(window.COUPcells*.35/10)*10/(Math.round(window.COUPcells*.35/10)*10+Math.round(window.CRpVIPnSPamidale/10)*10)*2000),
